...
impl<'a, T: 'a> StackVec<'a, T> {
    ...
    pub fn new(storage: &'a mut [T]) -> StackVec<'a, T> {
        let stackvec = StackVec{storage: storage, len: 0};
        stackvec
    }
    ...
    pub fn with_len(storage: &'a mut [T], len: usize) -> StackVec<'a, T> {
        if len > storage.len() {
            panic!("len is longer than storage length")
        }
        let stackvec = StackVec{storage: storage, len: len};
        stackvec
    }
...
impl<'a, T: Clone + 'a> StackVec<'a, T> {
    ...
    pub fn pop(&mut self) -> Option<T> {
        if self.is_empty() {
            ...
            let s = self.storage[self.len - 1].clone();
            self.len -= 1;
            Some(s)
        }
    }
